...
import { getRandom, repeat, fromCacheAsSprite } from '../utils';
...
export class FirstPartView extends PIXI.Container implements GarbageCollect {
  readonly name = 'first-part-container';
  ...
  private readonly _stateStartButtonSubject$ = new Subject<ButtonState>();
    ...
    this._garbageBag
      ...
      .subscribe(() => {
        ...
        this.startAnimation().then(() => {
          ...
          this._stateStartButtonSubject$.next(ButtonState.ACTIVE);
        });
      });
  }
  ...
  private startAnimation(): Promise<void> {
    ...
    this._stateStartButtonSubject$.next(ButtonState.UN_ACTIVE);
  ...
  get stateStartButton$(): Observable<ButtonState> {
